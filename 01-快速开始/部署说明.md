...
$dirs = @(
    ...
    "$PROJECT_ROOT\workspace\执行日志\allure-results",
    "$PROJECT_ROOT\workspace\执行日志\jmeter-results",
    "$PROJECT_ROOT\workspace\执行日志\jmeter-report",
    "$PROJECT_ROOT\workspace\执行日志\coverage-report",
    "$PROJECT_ROOT\workspace\执行日志\baselines",
    "$PROJECT_ROOT\workspace\执行日志\history",
    "$PROJECT_ROOT\workspace\执行日志\截图",
    "$PROJECT_ROOT\workspace\执行日志\报告"
)
...
curl -fsSL https://raw.githubusercontent.com/myorg/Test-Agent/develop/install.sh | bash -s -- ./my-project
```
...
## V1.1.0-alpha · 运行时层 `runtime/` 部署

> 运行时层是可选的(用户也可只用 14 专家+13 Skill+49 脚本的 V1.0.0 工作流模式)。
