"""Runtime prompt injection scan (hermes critical).

Charter rule: 非交互自动批准模式下,assembled prompt(含动态加载的 skill 内容)
必须全扫,不止 create-time。
"""
...
def scan(text: str) -> None:
    ...
    Charter rule: scan FULL assembled prompt (system + user + tools + skill contents).
    """
