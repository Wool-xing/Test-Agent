...
def test_registry_skill_status_counts():
    """Skill 32 = 23 production + 7 script + 0 rollout + 2 vision (V1.32.3 全 skill rollout 完成)。"""
    cat = get_catalog()
...
def test_router_does_not_falsely_flag_production_skill():
    """V1.32.3 全 rollout 完成 — production skill 不应被 flag 为 rollout/vision。"""
    cat = get_catalog()
...
def test_execute_node_allows_production_skill():
    """V1.32.3 全 rollout 完成 — production skill 应正常执行 (rc=0),不被硬拒。"""
