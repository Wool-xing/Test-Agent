...
覆盖 ROADMAP Day 0 承诺:rollout / vision / unknown 状态的 expert / skill,
...
def test_registry_expert_status_counts():
    """Expert 16 = 11 production + 5 script + 0 rollout 。"""
    cat = get_catalog()
    ...
    assert counts.get("rollout", 0) == 0, f"expert rollout 应 0 (rollout 收尾),实 {counts.get('rollout')}"
...
def test_registry_skill_status_counts():
    ...
    assert counts.get("vision", 0) == 0, f"skill vision 应 0 (后),实 {counts.get('vision')}"
...
def test_router_flags_rollout_expert():
    # rollout 收尾,所有 expert production/script。
