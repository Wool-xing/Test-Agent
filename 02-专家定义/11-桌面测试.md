...
| 文件 | 用途 |
|------|------|
| `workspace/执行日志/desktop-screenshots/*.png` | 桌面失败截图 |
| `workspace/执行日志/win-event-log/*.evtx` | Windows Event 日志 |
| `workspace/执行日志/mac-console/*.log` | macOS Console 日志 |
| `workspace/执行日志/desktop-perf/*.json` | psutil 进程性能 |
