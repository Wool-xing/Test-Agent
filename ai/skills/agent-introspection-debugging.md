...
description: Agent 自省调试 Skill。LLM 决策 / 工具调用 / 上下文 / token / 状态机透明化。失败用例分析 + 决策回放。派生自 ECC 同名 skill 。
...
- 主-子 session 共享 cache → 子代理 aux_client 隔离失效
- 重复调相同 LLM(无 cache)→ Anthropic prompt cache 没设 ttl

## 融合

- 决策可追溯(本 skill 直接消费)
- 隔离 client(本 skill 检测违反)
- 横切可复现性(本 skill 必带 seed + snapshot)
