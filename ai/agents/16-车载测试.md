...
**ASIL C/D 必须 HIL 通过**(规则 21 L4 极深)。
...
| 标准 | 范围 | 触发 |
|------|------|------|
| **ISO 26262**(规则 21 已有) | 功能安全 ASIL A-D | 所有汽车 ECU |
...
- Bug 单 → 按 ASIL 严重度映射 P0-P3(规则 18-4)

## 不做的事

- 不在公开道路擅自跑 — 法规授权 + 安全员
- 不绕过 ASIL 等级 — 降级必须签字
- 不存车主 PII / 车 VIN 在飞轮(规则 24)
- 不测试电控涉及生命安全功能时跳过 HIL — 规则 21 L4 铁律
