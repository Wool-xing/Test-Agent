...
result = run_monkey(
    ...
    pct_anyevent=3,                     # 其他事件
    output_dir="workspace/测试报告/{项目名}/monkey",
)
...
run_monkey 自动调 `archive_logcat()` 归档 crash 时刻的 Android 日志到 `workspace/测试报告/{项目名}/logcat/`。
...
| `workspace/测试报告/{项目名}/ios-syslog/*.log` | iOS 日志 |
