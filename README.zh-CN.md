...
```bash
# 前提：安装 Python 3.x
# Git / Node.js 缺失时自动安装（winget / brew / apt / apk）
...
> Python 3.x 为运行前提。Git / Node.js 缺失时自动安装（winget / brew / apt / apk）。
>
...
> **Windows 用户**：如果 curl 报 `CRYPT_E_NO_REVOCATION_CHECK`，改用 PowerShell。推荐安装路径：`D:\Test-Agent`。
> ```powershell
