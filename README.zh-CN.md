...
├── agents/             ← Agent 定义（AI 模式）
├── skills/             ← 32 业务 Skill + 3 元 Skill
├── utils/              ← 92 生产工具（pytest · Playwright · JMeter · Appium · …）
├── runtime/            ← 运行时引擎（router · orchestrator · MCP · learning · intelligence）
├── engine/             ← Rust 核心（DAG executor · 沙箱 · PyO3 bridge）
├── sdk/                ← Plugin SDK（scaffold · validate · install）
├── apps/desktop-v2/    ← Tauri 2 桌面应用
├── docs/site/          ← VitePress 文档站点
├── config/             ← conftest / pytest.ini / .mcp.json
├── ci/                 ← GitHub Actions（18 jobs，三平台矩阵）
├── install.py          ← 跨平台一键部署
└── README.md
```
