...
> 已有 16 专家 / 32 Skill / 74 utils**不动**(宪章铁律),`runtime/` 作可执行调度层 + 真 LLM-driven agent/skill runner。
...
### 与 16 专家 / 32 Skill / 74 utils 的关系

| 项 | 关系 |
|----|------|
| 16 专家 `.md` | **不动**。`registry` 扫 frontmatter,`router` 喂 LLM 选用 |
| 32 Skill `.md` | **不动**。同上 |
| 74 utils `.py` | **不动**。`orchestrator/adapters/scripts.py` subprocess 隔离调用 |
...
- V1.0.0 工程基线：14 agent + 14 skill + 74 utils + CI/CD（历史基线）
