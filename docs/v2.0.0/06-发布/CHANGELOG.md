...
### Sprint 3-6 新增 (2026-06-21)

#### Sprint 3 — 扩展体系
- **Skill SDK** — scaffold/validate/package/publish/discovery (29 TDD)
- **Skill市场** — 本地marketplace(publish/search/list) + 3示例Skill
- **MCP集成** — tagent mcp list/info, McpClient基础设施
- **skill CLI** — install/test/list/search 命令

#### Sprint 4 — B端/C端分化
- **deployment_mode** — enterprise/community切换
- **SSO OIDC** — SSOConfig+SSOManager (JWT签名验证, JWKS)
- **RBAC** — Role(4级)+Permission(7权限)+RBAC.has_permission
- **审计日志** — log_event JSONL写入+query_events查询
- **C端安装 4/4** — brew+pip+npm+scoop
- **Web Dashboard** — Playwright真浏览器验证+截图

#### Sprint 5 — 测试能力全覆盖
- **E2E双引擎** — Playwright(check_page) + Cypress(cypress run)
- **Visual回归** — screenshot capture+compare (PIL/numpy)
- **Integration** — API check + DB query (SELECT-only安全)
- **Unit executor** — pytest runner with coverage
- **Mobile** — Appium executor (check_installed+run_test)
- **Cron调度** — scheduler 815行 (run_job/tick/run_forever)
- **Pentest** — 5技能模块验证

#### Sprint 6 — 企业级完善
- **多LLM Provider** — claude/openai/deepseek/gemini/ollama/stub
- **报告系统** — HTML/JSON/JUnit/PDF 4格式
- **通知系统** — Slack/Email/Webhook 3渠道
- **知识图谱** — intelligence模块集成

#### 安全修复
- SSO JWT签名验证 (CRITICAL)
- Webhooks签名fail-closed (Discord/DingTalk/QQ)
- Webhooks防重放 (5分钟timestamp窗口)
- Visual路径穿越防护 (_safe_name)
- Integration SQL注入防护 (SELECT-only)
- test_runner路径穿越防护 (kebab-case验证)
- credential sanitization共享函数

#### 质量体系
- §三-C深审: 11/364文件
- 代码审查: 7轮code-reviewer + 3轮security-reviewer
- /loop: 5轮广度×深度扫描
- 竞品周报: Sprint 0-4全覆盖
- 四维达成度: 55/55/60/55%
- 技术债务: 17项延期追踪
