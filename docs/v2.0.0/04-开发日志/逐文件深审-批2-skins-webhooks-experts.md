...
| WR-002 | HIGH | :38 Discord/QQ未校验timestamp新鲜度, 无防重放 | 5分钟窗口校验 | ⬜ |
| WR-003 | HIGH | :491 _process_async_with_reply未实际回复 | 实现reply发送 | ⬜ |
| WR-004 | MEDIUM | :440 异常exc直接拼入用户回复 | _sanitize_error脱敏 | ⬜ |
...
| ER-002 | MEDIUM | :268 script kind绕过_impl_status门禁 | 扩展kind检查 | ⬜ |
...
| B2-002 | HIGH | webhooks.py | WR-003: _process_async_with_reply实现回复 | ⬜ |
