...
description: 禅道 Bug 提交技能。输入 Bug 描述或测试失败信息，自动规范化 Bug 报告并提交到禅道，支持批量提交和状态追踪。底层调用 utils/zentao_bug_manager.py（severity 1=P0/2=P1/3=P2/4=P3）。
...
with open("workspace/执行日志/regression_summary.json", encoding="utf-8") as f:
    results = json.load(f)
