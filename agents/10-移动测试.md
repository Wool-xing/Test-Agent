...
result = run_monkey(
    ...
    pct_anyevent=3,                     # 其他事件
    output_dir="workspace/测试报告/monkey",
)
...
run_monkey 自动调 `archive_logcat()` 归档 crash 时刻的 Android 日志到 `workspace/测试报告/logcat/`。
...
# 输出 workspace/测试报告/mobile-perf/{package}_{timestamp}.json
...
| 文件 | 用途 |
|------|------|
| `workspace/测试报告/mobile-perf/*.json` | 性能采集 |
| `workspace/测试报告/screenshots/mobile_*.png` | 移动失败截图 |
| `workspace/测试报告/logcat/*.log` | Android 日志 |
| `workspace/测试报告/ios-syslog/*.log` | iOS 日志 |
