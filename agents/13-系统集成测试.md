...
def test_device_uptime():
    ...
    import os
    with SSHClient(
        host=os.getenv("IOT_SSH_HOST"),
        user=os.getenv("IOT_SSH_USER", "root"),
        password=os.getenv("IOT_SSH_PASSWORD"),
    ) as ssh:
        out = ssh.exec("uptime")
...
| `workspace/测试报告/{项目名}/mq-logs/*.log` | MQ 投递/消费日志 |
